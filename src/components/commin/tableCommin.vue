<template>
  ...
  >
    <template v-for="(tableItem, tableIdx) in tableDataJson.typeData">
      ...
      />
      <el-table-column
        v-else-if="tableItem.types == 'expand'"
        :key="tableIdx"
        type="expand"
        :align="tableItem.align || 'center'"
        :label="tableItem.label"
      ...
      >
        <span slot-scope="scoped">
          ...
          <div v-else-if="tableItem.flag == 'radio'">
            <div
              class="weiduBox inline mr7"
              :class="!scoped.row[tableItem.types] ? 'weiduClick' : 'yiduClick'"
            ></div>
            <div class="inline">
              {{ !scoped.row[tableItem.types] ? " 未读" : "已读" }}
            </div>
          </div>
