<template>
	<div>
		<headerKuanJia @clickQueryUser="clickQueryUser" left-width="89%" @clearInputAll="clearInputAll">
			<template>
				<div class="displayalign mb16 zujianBox">
					<div class="noneIconTitle mr11">委托公司:</div>
					<div class="mr20 w320">
						<el-select v-model="paras.orgName" slot="prepend" :placeholder="EntrustedCompany.placeholder" @input="getOrdName"
						 @focus="getOrderArr">
							<el-option v-for="(item, idx) in EntrustedCompany.entrustedCompanyArr" :key="idx" :label="item.orgFullName"
							 :value="idx"></el-option>
						</el-select>
					</div>
				</div>
				...
				<div v-show="!_isRuku()" class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="w160">
							<inputHeader @changereplenishOrderNo="outWareNoFn" :buhuodanhaoJson="outWareNoJson"></inputHeader>
						</div>
					</div>
				</div>
				<!-- 出库单号 -->
				<div class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 w160">
							<el-autocomplete v-model="associatedOrderNo.associatedOrderNoCenter" :fetch-suggestions="associatedArr"
							 :placeholder="associatedOrderNo.placeholder" @select="associatedSelect"></el-autocomplete>
						</div>
					</div>
				</div>
				<!-- 关联单号 -->
				...
				<div v-show="_isRuku()" class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 w160">
							<el-autocomplete v-model="stockInNo.stockInNoCenter" :fetch-suggestions="stockInNoArr" :placeholder="stockInNo.placeholder"
							 @select="stockInNoSelect"></el-autocomplete>
						</div>
					</div>
				</div>
				<!-- 入库单号 -->
				<div class="displayalign mb16 zujianBox">
					<div class="noneIconTitle mr11">{{_isRuku()?'入库状态':"出库状态"}}:</div>
					<div class="mr20 w160">
						
						<el-select v-model="paras.putstatus" slot="prepend" :placeholder="putstatusJson.placeholder" @input="getPutstatus">
							<el-option v-for="(item, idx) in _isRuku()?putstatusJson.putstatusArr:outWareStatusJson.putstatusArr " :key="idx" :label="item.putstatusCenter" :value="idx"></el-option>
						</el-select>
					</div>
				</div>
				<!-- 入库状态 -->
				...
				<div class="displayalign mb16 zujianBox">
					<div class="noneIconTitle mr11">{{_isRuku()?'入库类型':"出库类型"}}:</div>
					<div class="mr20 w160">
						<el-select v-model="WarehousingType.WarehousingTypeCenter" disabled slot="prepend">
							<el-option v-for="(item, idx) in WarehousingType.putstatusArr" :key="idx" :label="item.putstatusCenter" :value="idx"></el-option>
						</el-select>
					</div>
				</div>
				...
				<div class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 w160">
							<el-autocomplete v-model="paras.prodCode" :fetch-suggestions="ProductCodeArr" :placeholder="ProductCodeJson.placeholder"
							 @select="ProductCode"></el-autocomplete>
						</div>
					</div>
				</div>
				<!-- 产品编码 -->
				...
				<div class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 w160">
							<el-autocomplete v-model="paras.prodId" :fetch-suggestions="ProductNameArr" :placeholder="ProductNameJson.placeholder"
							 @select="ProductName"></el-autocomplete>
						</div>
					</div>
				</div>
				<!-- 产品名称 -->
					...
					<div class="displayalign zujianBox">
						...
						<div class="mr20 displayalign">
							<div style="margin-right: 10px">
								<dateTime :dateTimeData="expectedSendStartTimeData" @getDateTime="getexpectedSendStartTime" ref="expectedStart" />
							</div>
							<!-- 开始时间 -->
							<div class="line"></div>
							<div>
								<dateTime width="210" :dateTimeData="expectedSendEndTimeData" @getDateTime="getexpectedSendEndTime" ref="expectedEnd" />
							</div>
							<!-- 结束时间 -->
						</div>
					</div>
				...
				<div v-show="_isRuku()" class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 displayalign">
							<div style="margin-right: 10px">
								<dateTime :dateTimeData="putStartTimeData" @getDateTime="getputStartTime" ref="putStart" />
							</div>
							<!-- 开始时间 -->
							<div class="line"></div>
							<div>
								<dateTime :dateTimeData="putEndTimeData" @getDateTime="getputEndTime" ref="putEnd" />
							</div>
							<!-- 结束时间 -->
						</div>
					</div>
				...
				<div v-show="!_isRuku()" class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 displayalign">
							<div style="margin-right: 10px">
								<dateTime :dateTimeData="putStartTimeData" @getDateTime="outWareTimeStart" ref="chukuStart" />
							</div>
							<!-- 开始时间 -->
							<div class="line"></div>
							<div>
								<dateTime :dateTimeData="putEndTimeData" @getDateTime="outWareTimeEnd" ref="chukuEnd" />
							</div>
							<!-- 结束时间 -->
						</div>
					</div>
				...
				<div v-show="!_isRuku()" class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 displayalign">
							<div style="margin-right: 10px">
								<dateTime :dateTimeData="putStartTimeData" @getDateTime="pickTimeStartTime" ref="jianhuoStart" />
							</div>
							<!-- 开始时间 -->
							<div class="line"></div>
							<div>
								<dateTime :dateTimeData="putEndTimeData" @getDateTime="pickTimeEnd" ref="jianhuoEnd" />
							</div>
							<!-- 结束时间 -->
						</div>
					</div>
				...
				<div v-show="!_isRuku()" class="zujianBox mb16">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 displayalign">
							<div style="margin-right: 10px">
								<dateTime :dateTimeData="putStartTimeData" @getDateTime="checkTimeStart" ref="fuheStart" />
							</div>
							<!-- 开始时间 -->
							<div class="line"></div>
							<div>
								<dateTime :dateTimeData="putEndTimeData" @getDateTime="checkTimeEnd" ref="fuhetEnd" />
							</div>
							<!-- 结束时间 -->
						</div>
					</div>
...
<script>
	...
	import inputHeader from "../headerCommin/inputHeader.vue"
	export default {
		components: {
			dateTime,
			headerKuanJia,
		...
		methods: {
			...
			//委托公司
			getOrdName(e) {
				this.paras.orgId = this.EntrustedCompany.entrustedCompanyArr[
					e
				].id;
			},
			//关联单号
			...
			getPutstatus(e) {
				let str = this._isRuku()?'putstatus':'outWareStatus'
				 // _isRuku()?putstatusJson.putstatusArr:outWareStatusJson.putstatusArr 
				 this.paras[str] = e
			}, //获取入库状态
			//产品编码
			...
			clearInputAll() {
				...
				this.outWareNoJson.input = ''
				this.paras.orgName = ''
				this.paras.orgId = ''
				this.paras.orderNo = ''
				this.paras.putWareNo = ''
				this.paras.putstatus = ''
				this.paras.outWareStatus = ''
				this.paras.prodCode = ''
				this.paras.outWareStatus = ''
				this.paras.prodName = ''
				this.paras.specName = ''
				this.paras.outWareNo = ''
				this.paras.putStartTime = ''
				this.paras.putEndTime = ''
				this.paras.expectedSendStartTime = ''
				this.paras.expectedSendEndTime = ''
				...
				this.$emit('clearInput', this.paras)
			},
		},
	};
</script>
