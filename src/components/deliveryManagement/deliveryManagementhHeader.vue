      ...
      <div class="biaogeBox">
        ...
        <div class="zujianBox mb20">
          <div class="displayalign zujianBox">
            ...
            <div class="mr20">
              <el-autocomplete
                v-model="addressJson.addressJsonCenter"
                :fetch-suggestions="addressJsonArr"
                placeholder="请输入产品名称"
                @select="addressJsonSelect"
              ></el-autocomplete>
              <el-select
                v-model="paras.address"
                slot="prepend"
                placeholder="请选择产品名称"
                @input="changeaddress"
                @focus="getaddress"
              >
                <el-option
                  v-for="(item, idx) in addressJson.addressArr"
                  :key="idx"
                  :label="item.address"
                  :value="idx"
                ></el-option>
...
export default {
  ...
  methods: {
    //获取数组
    async _getMes() {
      let { result } = await pDeliverGoodsFindNormalRecordPage(
        this.sendoutData
      );
    ...
    _changDianDanLaiYuan(res) {
      ...
      // this.orderContactPhone.orderContactPhoneCenter = res;
      // this.orderAddr.orderAddrCenter = res;
