	...
	<div id="headerMain">
		<div class="navMain">
			<div class="tabContainer">
					...
					<el-tabs type="card" v-model="activeName" @tab-click="handleClick">
						<el-tab-pane v-for="(navIndex, idx) in dataArr" :key="idx">
							<div slot="label" v-if="navIndex.children">
								<el-dropdown placement="bottom" trigger="click" class="displayalign" v-if="navIndex.children.length > 1"
								 @command="clickEventGoRouter" ref="mouseoutHidden">
									<span class="el-dropdown-link">
										<img class="navMainDropdownImgBox" :src="navIndex.iconCls" width="16" height="16" />
										<span class="lineHeight34">{{ navIndex.title }}</span>
									</span>
									<el-dropdown-menu slot="dropdown">
										<el-dropdown-item v-for="(itemCenter, idx) in navIndex.children" :key="idx" :command="idx">{{ itemCenter.title }}
										</el-dropdown-item>
									</el-dropdown-menu>
								</el-dropdown>
								<div v-else>
									<span class="el-dropdown-link">
										<img :src="navIndex.iconCls" class="navMainDropdownImgBox" width="16" height="16" />
				...
				<div class="el-lr">
					<div class="el-icon-arrow-left left" @click="leftMove" :style="{
              opacity:
                leftMoveClick && this.steep == this.navWidth - this.innersWidth
                  ? 0.46
                  : 1,
            }"></div>
					<div class="el-icon-arrow-right right" @click="rightMove" :style="{
              opacity:
		...
		<div class="dianjiqiehuan" v-show="dropdownArr.length" id="clickBox">
			<div class="dianjiCenters">
				<el-tabs v-model="activeTabsName" type="card" @tab-remove="removeTab" @tab-click="handleTabsEdit">
					<el-tab-pane :key="idx + 'dsadsa'" v-for="(item, idx) in dropdownArr" :closable="item.title != '首页'" :label="item.title"
					 :name="'' + idx">
					</el-tab-pane>
				</el-tabs>
			</div>
		...
		<div class="dianjiqiehuan" v-show="mianbaoxieArr.length >= 2 && mianbaoxieArr[0].title !== mianbaoxieArr[1].title">
			<div class="mianbaoxie" v-show="!isZhanneixiaoxi">
				<el-breadcrumb separator-class="el-icon-arrow-right">
					<el-breadcrumb-item v-for="(item, idx) in mianbaoxieArr" :key="idx + 'dsa'" :to="{ path: item.name }">{{ item.title }}
					</el-breadcrumb-item>
				</el-breadcrumb>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			isZhanneixiaoxi = sessionStorage.getItem('isZhanneixiaoxi') ? JSON.parse(sessionStorage.getItem('isZhanneixiaoxi')) :
				false
