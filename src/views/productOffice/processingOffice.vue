<template>
  <div>
    ...
    ></processingHeader>
    <!-- 头部-->
    <centerBtnArr title="加工作业" :btnArr="btnArr"> </centerBtnArr>
    <!-- but按钮 -->
    ...
    <div class="bjBox" v-if="isShowShop">
      <createPurchasing
        :isCankuzuoye="true"
        @closeFn="purchasingClose"
      ></createPurchasing>
    </div>
    <!--采购单 end-->
    <div class="bjBox" v-if="zichankudetails">
      <warehouseSure
        @closeDetails="closeChuKuFun"
        :isLooker="true"
      ></warehouseSure>
    </div>
    <!--出库-->
...
export default {
      ...
      btnArr: [
        {
          ...
          onClick() {
            if (
              !self.tableDataJson.dataResult.length ||
              self.tableDataJson.dataResult.length != 1
            )
              return self.$messageSelf.message({
                type: "warning",
  ...
  methods: {
    ...
    clickSubmit(json) {
      if (this.isEdit) {
        ...
        this.$pOrgProductsApp
          .pProcessWorkWarePlanSaveRecord(json)
          .then((res) => {
            if (res.code === "10000") {
              this.isShowChedule = false;
    ...
    async getDetailDatas(id) {
      let data = await this.$pOrgProductsApp.pProcessWorkWarePlanFindRecord({
        id,
      });
    ...
    async getTableData() {
      let data = await this.$pOrgProductsApp.pProcessWorkFindRecordPage(
    ...
    async _editpProcessWorkWarePlanEditRecordFun(json) {
      let data = await this.$pOrgProductsApp.pProcessWorkWarePlanEditRecord(
        json
      );
      if (data.code === "10000") {
        this.$messageSelf.message({
    ...
    _delData(arr) {
      this.$messageSelf
        .confirms(this.$clearArr(arr.ids), "提示", {
          type: "warning",
        })
        .then(async () => {
          let data = await this.$pOrgProductsApp.pProcessWorkWarePlanDelRecord(
            arr
          );
          if (data.code == "10000") {
            this.$messageSelf.message({
    ...
    _otherWancheng(arr) {
      this.$messageSelf
        .confirms(this.$clearArr(arr.ids, "强制完成"), "提示", {
          type: "info",
        })
        .then(async () => {
