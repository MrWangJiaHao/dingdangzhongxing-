<template>
	<div class="userSettingBox mb16">
		<div class="fuzzyQueryBox clearfix mb16">
			<div class="biaogeBox dispalywrap fl">
				<div class="displayalign zujianBox">
					...
					<div class="mr20 w160">
						<el-select v-model="pagingQueryData.paras.childWareName" slot="prepend" placeholder="请选择子仓名称" @input="getwareHouseValue">
							<el-option v-for="(item, idx) in nameOfSubwareHouseDataArr" :key="idx" :label="item.childWareName" :value="idx"></el-option>
						</el-select>
					</div>
				</div>
				<!-- 子仓名称 -->
				<div class="zujianBox">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 w150">
							<el-select v-model="wareAreaTypeJson.wareAreaName" placeholder="请选择区域类型" @input="getAreaValue">
				...
				<div class="zujianBox">
					<div class="displayalign zujianBox">
						...
						<div class="mr20 w160">
							<el-select v-model="pagingQueryData.paras.wareAreaName" slot="prepend" placeholder="请选择区域名称" @change="getAreaNameValue"
							 @focus="getQuYuData">
								<el-option v-for="(item, idx) in AreaName.AreaNameData" :key="idx" :label="item.wareAreaName" :value="idx"></el-option>
							</el-select>
						</div>
					</div>
				</div>
		...
		<div v-show="quyuLook" class="bjBox">
			<div v-show="quyuLook" class="pofixCenter displayCenter">
				<transition enter-active-class="animate__animated animate__zoomIn" leave-active-class="animate__animated animate__zoomOut">
					<div v-if="quyuLook" class="quyupinmianBoxMsg">
						<kuanjiaClick titles="区域平面图" @clickSubmit="clickQuYuClick" @closeBtn="closeQuYu" min-height="450" max-height="760"
						 :isLookerShow="false" print="关闭">
							<template>
                              ...
                              "
														 slot="prepend" :placeholder="nameOfSubWareHouse.placeholder" @input="getZicankuOfSunWareHouseWareId">
															<el-option v-for="(
                                  item, idx
                                ) in quyuzicanNameOfSubWareHouse.nameOfSubwareHouseData"
															 :key="idx" :label="item.childWareName" :value="idx"></el-option>
														</el-select>
													</div>
												</div>
											</div>
											<div class="displayalign">
												<div class="noneIconTitle mr11">区域类型:</div>
												<el-radio v-show="quyuleixingMsg == 1" v-model="quyuleixingMsg" label="1">存储区</el-radio>
												<el-radio v-show="quyuleixingMsg == 2" v-model="quyuleixingMsg" label="2">拣货区</el-radio>
											</div>
										</div>
										<div class="bianjiUser" @click="dinjichaxunquyutu">
											查询
										</div>
									</div>
								</div>
		...
		<div v-show="kuwieLook" class="bjBox">
			<transition enter-active-class="animate__animated animate__zoomIn" leave-active-class="animate__animated animate__zoomOut">
				<div v-if="kuwieLook" class="pofixCenter displayCenter">
					<kuanjiaClick titles="库位平面图" @clickSubmit="kuwieLook = !kuwieLook" @closeBtn="quyuLook = kuwieLook = false"
					 min-height="450" max-height="760" :isLookerShow="false" print="关闭">
				...
				<div class="center">
					<el-table ref="multipleTable" :data="tableData" :stripe="true" :border="true" tooltip-effect="dark" style="width: 100%"
					 @selection-change="handleSelectionChange">
						...
						<el-table-column label="序号" type="index" width="71" align="center" show-overflow-tooltip />
						<el-table-column label="子仓名称" prop="childWareName" width="120" show-overflow-tooltip />
						<el-table-column label="区域类型" property="wareAreaType" show-overflow-tooltip></el-table-column>
						<el-table-column width="130" label="区域名称" prop="wareAreaName" show-overflow-tooltip></el-table-column>
						<el-table-column width="119" label="区域长(m)" align="center" prop="wareAreaLength" show-overflow-tooltip></el-table-column>
						<el-table-column label="区域宽(m)" align="center" width="119" prop="wareAreaWidth" show-overflow-tooltip></el-table-column>
						<el-table-column label="货架总排数" width="119" align="center" prop="rowNum" show-overflow-tooltip></el-table-column>
						<el-table-column label="货架总数" align="center" width="119" prop="shelfNum" show-overflow-tooltip></el-table-column>
						<el-table-column label="货架总库位数" align="center" width="119" prop="seatNum" show-overflow-tooltip></el-table-column>
						<el-table-column label="已用库位" width="119" prop="useSeat" align="center" show-overflow-tooltip></el-table-column>
						<el-table-column label="未使用库位" width="119" align="center" prop="noSeat" show-overflow-tooltip></el-table-column>
						<el-table-column label="创建人" width="120" align="center" prop="createUser" show-overflow-tooltip></el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			closeQuYu() {
				this.quyuLook = false;
				this.quyuzicanNameOfSubWareHouse.nameOfSubWareHouseName = "";
			},
			clickQuYuClick() {
				this.closeQuYu();
			},
			...
			async getZicankuOfSunWareHouseWareId(e) {
				...
				this.quyuMsg =
					`${this.quyuzicanNameOfSubWareHouse.nameOfSubwareHouseData[e].tails.wareName}/${this.quyuzicanNameOfSubWareHouse.nameOfSubwareHouseData[e].wareAreaType}`;
				console.log(
					this.quyuzicanNameOfSubWareHouse.nameOfSubwareHouseData[e].wareAreaType
				);
				this.quyuleixingMsg =
					this.quyuzicanNameOfSubWareHouse.nameOfSubwareHouseData[e]
					.wareAreaType == "存储区" ?
					"1" :
					"2";
			...
			createSubWarehouse() {
				if (this.multipleSelection.length == 0)
					return this.$messageSelf.message({
						message: "请选择在那个子仓下创建区域",
						type: "warning",
					});
				if (this.multipleSelection.length > 1)
					return this.$messageSelf.message({
						message: "每次只能在一个子仓下创建区域",
						type: "warning",
					});
				this._getChildWidth({
					wareId: getCookie("X-Auth-wareId"),
			...
			async createWarehouseAjax(data, path) {
				...
				if (datas.code === "10000") {
					...
					if(path == '/warehoseconfig/editWarehouseConfig'){
						localStorage.setItem(
							"createWareHuseData",
							JSON.stringify({
								childWareId: this.multipleSelection[0].childWareId,
								id: this.multipleSelection[0].id,
								wareAreaName: this.multipleSelection[0].wareAreaName,
