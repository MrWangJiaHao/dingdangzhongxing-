<template>
  <div class="shelfSettingBox">
    ...
    <!-- js 主体部分 -->
    <div v-show="sendOutData.rowData[0].groupData.length" class="backFF p20">
      <div class="disRight">
...
export default {
  ...
  methods: {
    ...
    async _mingc(data, fn) {
      let datas = await post({
        url: "/wbs-warehouse-manage/v1/pWarehouseShelf/findRecord",
        data,
      });
      if (datas.code == "10000") {
      ...
      this.sendOutData.rowData.forEach((item, idx) => {
        ...
        item.groupData.forEach((groupData, idxs) => {
          if (item.groupData.length >= 2 && groupData.distance === undefined) {
            str = `请输入第${this.isNums(idxs)}/${this.isNums(
              idxs + 1
            )}组货架间距`;
            return (show = false);
          }
    ...
    async submitDataJson(data) {
      let datas = await post({
        url: "/wbs-warehouse-manage/v1/pWarehouseSeat/saveRecord",
        data,
      });
      if (datas.code == "10000") {
