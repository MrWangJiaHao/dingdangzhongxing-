...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    //from.name === "/deliveryManagement/normalDelivery" && this._jijiJianhuodan(this)
    next();
  },
  methods: {
    //拣货单 作废
    TovoidClick(e) {
      ...
      this.$messageSelf
        ...
        .catch(() => {
          this.$messageSelf.message({ type: info, message: "已取消作废" });
        });
    },
    _TovoidClick() {
      pOrgPickOrderPickCancle([
    ...
    _jijiJianhuodan(id) {
      pDeliverGoodsUpdatePrintExprStatus([
        ...
        .then((res) => {
          if (res.code == "10000") {
            this.multipleSelection[0].pickOrderNo = id;
