...
export default {
  ...
  methods: {
    ...
    warehousingConfirmation() {
      ...
      this.$messageSelf
        .confirms(
          ` 共集计${this.multipleSelection.length}个订单，可生成
                        ${this.multipleSelection.length}张拣货单，确认集计吗？
                        <div id='checkboxID'> 
                        <input checked  id='checkbox' type='checkbox' /> 打印集计单</div>`,
          "集计确认",
          {
            showClose: true,
        ...
        .then(() => {
          this.iSJianHuoDan(); //打印拣货单
          this._jijiJianhuodan(); //集计单
        });
      // .catch(() => {
