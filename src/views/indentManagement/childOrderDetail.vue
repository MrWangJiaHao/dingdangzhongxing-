	...
	<div class="childOrderDetailMain">
		<div class="orderDetaiPage">
			...
			<div class="orderDetailBox">
				...
				<el-table :data="tableData" border style="width: 100%" :stripe="true" tooltip-effect="dark">
				...
				<div class="timeLineBox">
					<ul>
						<li v-for="(v, i) in timeLineArr" :key="i">
							...
							<div class="rightLine" :style="{ background: v.background }"></div>
						</li>
					</ul>
				</div>
				<el-table :data="tableData1" border style="width: 100%" :stripe="true" tooltip-effect="dark">
					<el-table-column label="序号" align="center" type="index" width="71">
					</el-table-column>
				...
				queryFun: () => {
					...
					childOrderInfor(childOrderQuery).then((ok) => {
						if (ok.data.code === "10000") {
							...
							if (ok.data.result.length) {
								let type = this.tableData1[this.tableData1.length - 1].operateType;
								console.log(this.tableData1[this.tableData1.length - 1].operateType)
								if (type <= 3 || type === 4) {
									this.val = 1;
