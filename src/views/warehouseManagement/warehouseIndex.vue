			...
			<div class="btnArr">
				<div class="backFF">
					...
					<div class="btnClick">
						...
						<a class="daochuMes disinb" :href="
                baseUrl + '/wbs-warehouse-manage/v1/pOutWarehouse/exprotExcel'
              ">导出</a>
						<span v-if="$route.params.type == 0" class="mr10"></span>
						<div class="goOn" v-if="$route.params.type == 0" @click="CreateStockInOrder">
							创建
						</div>
						<div class="lodopFunClear" v-if="$route.params.type == 0" @click="editBtn">
							编辑
						</div>
						<div class="remove" v-if="$route.params.type == 0" @click="clearBtn">
							删除
						</div>
					</div>
				</div>
					...
					<div class="center tableInInput">
						<el-table ref="multipleTable" :data="tableData" :stripe="true" :border="true" tooltip-effect="dark" style="width: 100%"
						 @selection-change="handleSelectionChange">
							<el-table-column type="selection" align="center" width="82" fixed="left">
							</el-table-column>
							<el-table-column label="序号" align="center" type="index" width="71" show-overflow-tooltip />
							<el-table-column label="委托公司" width="250" prop="orgName" show-overflow-tooltip />
							<el-table-column label="出库单号" width="250" show-overflow-tooltip>
							...
							<el-table-column width="119" label="关联单号" prop="orderNo" show-overflow-tooltip></el-table-column>
							<el-table-column width="119" label="出库状态" prop="outWareStatus" show-overflow-tooltip></el-table-column>
							<el-table-column width="119" label="物流公司" prop="exprName" show-overflow-tooltip></el-table-column>
							<el-table-column width="119" label="物流单号" prop="exprNo" show-overflow-tooltip></el-table-column>
							<el-table-column width="150" label="出库人" prop="outWareUser"></el-table-column>
							<el-table-column label="出库时间" width="160" prop="outWareTime"></el-table-column>
							<el-table-column label="拣货人" width="119" prop="pickUser" show-overflow-tooltip></el-table-column>
							<el-table-column label="拣货完成时间" prop="pickTime" width="200" show-overflow-tooltip></el-table-column>
							<el-table-column label="复核人" width="119" prop="checkUser" show-overflow-tooltip></el-table-column>
							<el-table-column label="复核完成时间" width="119" prop="checkTime" show-overflow-tooltip></el-table-column>
						</el-table>
					</div>
					<!-- 表格主体 -->
		...
		<div v-show="WarehouseReceipts" class="bjBox">
			<transition enter-active-class="animate__animated animate__zoomIn" leave-active-class="animate__animated animate__zoomOut">
				<div v-if="WarehouseReceipts">
					<WarehouseReceipt :WarehousingType="
		...
		<div v-show="isCreateWarehouse" class="bjBox">
			<transition enter-active-class="animate__animated animate__zoomIn" leave-active-class="animate__animated animate__zoomOut">
				<div v-if="isCreateWarehouse">
					<div>
						<createWarehouse :outWareType="sendOutDataJson.paras.outWareType" @closeBtns="closeBtns" :editCreateWarehouse="editCreateWarehouse" />
					</div>
				</div>
			</transition>
		</div>
		...
		<div v-show="isWarehouseSure" class="bjBox">
			<transition enter-active-class="animate__animated animate__zoomIn" leave-active-class="animate__animated animate__zoomOut">
				<div v-if="isWarehouseSure">
					<div>
						<warehouseSure :isWarehouseSureDetails="isWarehouseSureDetails" @closeDetails="closeDetails" />
					</div>
				</div>
			</transition>
		</div>
		...
		methods: {
			...
			warehousingConfirmation() {
				if (!this.multipleSelection.length) {
					return this.$messageSelf.message({message:"请选择出库确认的单号",type:"warning"})
				} else if (this.multipleSelection.length > 1) {
					return this.$messageSelf.message({message:"只能选择一个出库单号",type:"warning"})
				} else {
					this._getpOutWarehousefindOutWareDetailById(
						...
						if (res.code == "10000") {
							...
							this.$messageSelf.message({message:"获取出库确认失败，请与管理员联系",type:"warning"})
						}
					});
				}
			},
			...
			printstockinlist() {
				if (!this.multipleSelection.length) {
					return this.$messageSelf.message({message:"请选择要打印的出库单",type:"warning"})
				} else if (this.multipleSelection.length > 1) {
					return this.$messageSelf.message({message:"只能选择打印一个出库单",type:"warning"})
				} else {
			...
			editBtn() {
				if (!this.multipleSelection.length || this.multipleSelection.length != 1)
					return this.$messageSelf.message({message:"请选择要编辑的出库单，只能选择一个编辑出库单",type:"warning"})
				sessionStorage.setItem(
					"warehouseEdit",
			...
			clearBtn() {
				if (!this.multipleSelection.length || this.multipleSelection.length != 1)
					return this.$messageSelf.message({message:"请选择要删除的出库单，只能选择一个删除出库单",type:"warning"})
				this.$messageSelf
			...
			changeDatas(datas) {
				if (datas.list) {
					datas.list.forEach((item) => {
						item.outWareStatus = this.isPutStatus(item.outWareStatus) 
					});
				}
